...
2. `PA_STABLIZE.py` 存在，但主循环中的调用被注释掉；启动 I2C 扫描未发现 MPU6050 的 `0x68/0x69`。
3. 当前 Web 控制是阻塞式 HTTP GET 轮询，控制命令和网页服务都在 MicroPython 运行时中竞争资源。
4. 当前代码中的 I2C 引脚是 GPIO21/GPIO22；备用引脚是 GPIO18/GPIO19 和 GPIO32/GPIO33，必须结合实物再确认。
...
| MPU6050 | `PA_IMU.py` | 假设 0x68 | I2C 从设备 | 启动扫描未发现 0x68/0x69，必须先解决硬件 |
...
|---|---|---|---|
| P0 | 工程、日志、I2C、PCA9685 | PlatformIO ESP-IDF 工程可编译烧录 | I2C 扫描看到 0x40、0x41；每个舵机可单独安全动作 |
| P1 | 配置和逆运动学 | NVS、`kinematics.c` | Python/C 离线对照误差在容差内；配置重启保持 |
...
- ESP32 板子的准确型号和 GPIO 丝印，是否与代码中的 21/22 完全一致。
- 两个 PCA9685 的真实板号、电源接线、公共地和舵机供电。
- MPU6050 是否实际安装、供电和维护地址，当前扫描未发现。
- 机械臂三个舵机的真实通道，特别是源码注释和当前配置不一致的部分。
- Type-C 是只连接 ESP32，还是还参与其他供电/充电路径。
- 电池、BMS、降压、舵机母线的实际拓扑。
