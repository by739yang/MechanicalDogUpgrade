...
`web_common.py` 里这四层。**逐条查过，C 侧一个都没有**（现已全部补完，见下表状态栏）：

| 原版函数 | 行 | 作用 | C 侧 |
|---|---|---|---|
| `_joy_f_to_thr(vf)` | 70 | 摇杆百分比 → `thr`：`\|vf\|<10`→0（**严格小于**，±10 会真的产生 0.6）；`thr = vf*6.0/100`；再 `*= joy_fwd_sign`；夹到 `[-3.0, 6.0]`（用严格 `>`/`<`，所以 ±6.0/−3.0 原样通过） | ✅ `web_cmd_joy_f_to_thr` |
| `_thr_is_forward/backward(thr)` | 86/95 | 阈值 0.35，**两条都是严格比较**：符号为 −1 时前进 = `thr < -0.35`、后退 = `thr > +0.35`；**恰好 ±0.35 两边都不算**，落进"既不前进也不后退"那一支 `_go(0,L,R)` | ✅ `web_cmd_thr_is_*` |
| `apply_dog_stick(thr, turn, force)` | 111 | 见下 | ✅ `web_cmd_apply_dog_stick` |
| `process_dog_from_req(...)` | 142 | 见下 | ✅ `web_cmd_process_request` |
| `parse_dog_stick` / `_parse_pair` | 34/48 | 原版**没有 `&` 和 `?`** 的分词器（见 `(9)`） | ✅ `proto_decode_legacy()` |

⚠️ **`joy_fwd_sign = -1` 的来源（我一开始引错了）**：它**不是** `padog.py` 里定义的 ——
`padog.py:59` 的默认值是 **+1**，那个 **−1 来自 `micropython/config_s.py:56`**。
golden 环境里 `config_s.py` 是**真文件**，所以参考值确实是 −1、C 侧常量也是 −1。
但**引用必须指对**（P-24 的教训：别把"值对"当成"来源对"）。
⇒ 生成器现在会**硬断言** `padog.joy_fwd_sign == -1`，一旦有人改动它，
**整套测试直接中止** —— 因为这一层**所有的方向判断都挂在它身上**。

⚠️ **关于 20 与 10 两个阈值**：`apply_dog_stick` 用 `JOY_TURN_DEAD=20`，
而它调用的 `_turn_phase_lr()` 内部用 `HIP_TURN_DEAD=10`。**这个差别是真的，
但在可达输入上观察不到** —— 调相位映射那一支只在 `|t| >= 20` 时才进，
而 `20 > 10`，两个阈值在**每一个可达输入**上结论都一致。
⇒ 实现**照原样**（`t` 不做额外夹取），并在 `web_cmd.c` 里注明
"这条差别正确但不可观察"，免得后人以为写错了去**统一**这两个常量。

⚠️ **另一处原文的不一致（§0.5(8) 原来没写）**：夹取只存在于 `_joy_f_to_thr` 里，
`_thr_is_forward/_backward` **收到的是没有夹取的 `thr`** —— 所以
"拿一个没夹取的 `thr` 去调 `apply_dog_stick`"在原版是个**真实的分歧点**。
C 版把它变成**可观察的前置条件**：`web_cmd_apply_dog_stick()` 对
`thr` 落在 `[-3, 6]` 之外返回 `WEB_CMD_ERR_THR_RANGE` 且**什么都不发**。
...
#### ⚠️(8b) **一处已知的行为分歧，需要接线时修掉**：网页"原地步态测试"不再只生效一帧

**原版（已在本项目文档里记过、且决定"忠实复刻"的怪癖）**：`inplace_step_end_ms`
是**模块级全局**，`web_c.py` 把它设成 `now+5000`，`process_dog_from_req` 看到它没过期
就发 `set_leg_sit_offsets(0,0)` + **`move(4,1,1)`**；而 `move()` 自己会因为
`(L+R)!=0 && |spd|>0` 把它**清零** ⇒ 这个分支**只生效一帧**，下一帧过期判断为假，
落回正常路径。**原版的"原地踏步 5 秒"在它自己内部就是不可达的。**

**C 版现状（分歧）**：那个截止时刻现在归**动作层**（`action_state_t`，
见 `control/action.h:57`），而链的 move 路径
（`control_chain_cmd_move` → `set_spd_lr`，`control_chain_cmd.c:71~80` / `:97~123`）
清的是 `control_chain_state_t` **自己那份** `inplace_step_end_ms`。
**全树没有任何地方清动作层那份** ⇒ 只要浏览器在轮询，`sit:0;0|move:4;1;1`
这个分支就**按页面频率一直触发**，而不是一次。

⚠️ 注意：**主循环那条路是对的** —— `action_inplace_step()` 自己会
`st->inplace_step_end_ms = 0`（`action.c:540`），所以"mainloop 版只生效一帧"
这个怪癖在 C 版**已经忠实复刻**。分歧只出在**网页这条路**上。

**决定：忠实复刻原版（只生效一帧）。** 理由：
1. 本项目的默认是"忠实"，而 `§8 迁移时必须修正的旧设计` 那张清单里**没有**这一条；
2. 原版那个 5 秒的*意图*在**原版自己内部就不可达**，所以"让它一直触发"等于
   **发明新行为**，不是"保留原意"；
3. 用户之前明确选择过"不擅自加东西、保持与原版一致"。

**接线时要做的**（属于 P5 传输层 / P6 接线那一步）：
在 `web_cmd_process_request()` 走完 inplace 分支的 `move(4,1,1)` 之后，
调用**已经存在**的 `app_action_set_inplace_step_end_ms(0)` 把动作层那份清掉
（这个访问器是搬爬行入口时补的）。
⚠️ `test_web_cmd` 目前把**当前**行为作为 INFO 打印并断言下来（免得未来某次修改
悄悄改掉它）；**修完之后那条断言要翻过来**，改成断言"只生效一帧"。
...
### (10) 协议层落地后的**已知缺口**（必须知道，否则现场会以为是 bug）

协议层已实现并全绿（`firmware/src/comm/proto.{h,c}`，纯 C、`proto.c` 只 include
`string.h`、无 `malloc`、`comm/` 下不含任何 ESP-IDF 头文件 —— 已 byte/grep 核实）。
但有四个**故意不覆盖**的地方，接线和实测时必须知道：

1. **标定页"更改控制器参数"那个表单在 C 版走不通（老路径会整帧拒绝）。**
   原版 `web_c.py` 里 `speed=` 会触发一整块 `exec()` 参数写入（20 项：
   `l1 l2 l b w speed h kp_h kp_g cg_x cg_y walk_h walk_speed trot_cg_{f,b,t}
   leg_len_ref joy_fwd_sign hip_k_{roll,pitch,turn} hip_delta_max`）。
   这些**不是 20~50 Hz 控制命令，而是配置写入**，所以**没有**做进控制帧。
   后果：老页面提交这个表单时，请求里全是**未知键** ⇒ 按"未知键整帧拒绝"的规则
   被丢掉，**机器人的动作部分是 0**（页面照常服务）。
   ⇒ 正确形状是**单独一个配置帧**：解成 `app_config_t` 后走那**唯一一处**
   `app_config_validate()`（控制台 `cfg set` 今天就能走到），**不要把范围再抄
   一份进 `proto.c`**（P-22/P-27）；§8.4 还要求它落 NVS。
   ⇒ 在做出这个配置帧之前，改参数一律走**串口 `cfg set` + `cfg save`**。
2. **老路径没有 `seq` ⇒ 没有重放保护。** 老页面根本不发 `seq`，这是真实残余缺口，
   也正是严格格式存在的理由。**页面改造完就应该关掉老入口。**
3. **页面路由不属于协议层**（`ss` 跳 `cal.html`、`speed` 触发整页、favicon、
...
的"可带 `+`、可有前导零、**忽略值尾部垃圾**"（`f=10abc` → 10），严格格式则要求
