...
> 阶段：**P0**（工程骨架 + 串口日志 + I2C 扫描 + PCA9685 单通道控制）
> 依据：`../ESP-IDF_C迁移表.md` 的阶段 P0；硬件事实来自《硬件实物核对清单.md》（该文件放在桌面）

---

## 1. 硬件基线（2026-09-14 实测）

| 项目 | 值 |
|---|---|
| 主控 | ESP32（**非** S3），4 MB Flash，CH340 → COM4 |
| 唯一 I2C 总线 | **SDA = GPIO21，SCL = GPIO22，100 kHz** |
| 舵机驱动 | PCA9685 **0x40**（左半身）、**0x41**（右半身）；`0x70` 是 all-call 广播地址 |
| IMU | **没有**（18/19、32/33 及全部引脚组合都扫不到 `0x68/0x69`） |
...
1. 初始化 I2C 总线
2. 扫描总线，核对 `0x40` / `0x41` 是否都在
3. 初始化两片 PCA9685（50 Hz），并把**全部通道置为「无脉冲」**（舵机松力）
4. 回读 `MODE1` / `PRESCALE` 并打印，与 MicroPython 版实测值对照
...
- [ ] 串口日志正常（能看到构建信息与自检输出）
- [ ] I2C 扫描看到 **0x40、0x41**
- [ ] 回读 `MODE1` / `PRESCALE`，与 MicroPython 版实测一致（`0x21` / `122`）
- [ ] 12 个逻辑通道可**逐个**安全动作（每个单独 `set`，先架空）
- [ ] `off` 能把整片板置为无脉冲
...
└── src/
    ...
    ├── app/
    │   └── app_p0.{h,c}        P0 自检 + 串口控制台
    ├── bsp/
    │   └── bsp_i2c.{h,c}       I2C 主机总线（GPIO21/22, 100 kHz）
    └── drivers/
        └── drv_pca9685.{h,c}   PCA9685 驱动
```

后续阶段按迁移表增加 `control/`（kinematics / gait / body_pose）、
`arm/`（arm_control）、`comm/`（wifi / http_server / websocket / telemetry）。
