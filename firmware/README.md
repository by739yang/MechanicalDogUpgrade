...
| `motion stat` | 周期、抖动、超期次数、I2C 写次数、12 路目标与当前角度 |
| `motion period <ms>` | 控制周期，默认 10（100 Hz），下一个周期即生效 |
| `motion rate <deg_per_s>` | 速率上限，默认 120 °/s —— 目标突变不会把舵机"甩"过去 |
| `motion timeout <ms>` | 命令超时后松力停车，默认 10000；`0` = 关闭 |
| `stand` | 目标设为**直接站姿**（12 路 = 中位角），由控制任务限速走过去 |
