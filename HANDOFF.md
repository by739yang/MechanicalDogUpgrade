...
4. MPU6050 驱动代码存在，但启动扫描**没有发现 0x68/0x69**。这是姿态闭环当前最重要的硬件阻塞项。
...
- 代码按 MPU6050 编写。
- 维护地址假设为 `0x68`，AD0 拉高时可能是 `0x69`。
- 当前启动扫描没有发现 `0x68/0x69`。
- 需要检查：模块是否实际安装、供电、SDA/SCL、AD0 电平、是否和 PCA9685 存在总线冲突。
