...
| App 层 | `app_chain` / `motion` 的状态机（模式切换、急停、超时、门控） | 🟡 | 仅编译 + review | **没有宿主测试**（第 0 步要补） |
| 驱动 | `drv_pca9685` 寄存器编解码 | 🟢 | 编码端被 `servo_map` golden 间接钉住；P-21 的 `&0x1F` 修正 🔵 真机 `readback` 验证 | 真实 I2C 时序未验 |
...
| **整条控制链**（配置→缩放层→slew→重心→IK→映射→I2C→芯片） | **⚪ 没验** | 板子不在手边；期望值已备好（`control_chain.csv` 第 1 行） |
