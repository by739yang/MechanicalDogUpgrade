...
| P3 | TROT / WALK 步态与姿态数学 | 🟡 **进行中** —— **控制链已迁移并全链路验证**：`control_chain.c`（纯 C）≈ 原版 `mainloop()` 的一帧；参考值是把**真版 `padog.py` exec 进来直接跑 `mainloop()`** 得到的，**90 行 / 1080 组占空比零容差相等**。包含此前从没迁过的"大狗缩放层"、姿态 slew 环、5 个重心分支、59 项默认值注入表。**待做**：补齐 15 个配置字段 → 接进固定周期任务 → 步态命令 → 硬件试跑 |
